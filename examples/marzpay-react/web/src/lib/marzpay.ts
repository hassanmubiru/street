...
//
// Built on the existing @streetjs/client fetch pattern (see src/main.tsx, which
// reads import.meta.env.VITE_API_URL and calls fetch(API_URL + path)).
// `initializePayment`/`verifyPayment` call the application's MarzPay endpoints
// exposed by the StreetJS backend (see ../../../src/main.ts). The browser holds
// NO MarzPay credentials — every MarzPay HTTP call happens on the backend.
//
