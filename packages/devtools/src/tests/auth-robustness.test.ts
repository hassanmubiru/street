...
const methodArb = fc.oneof(
  fc.constantFrom(...SAFE_METHODS, 'GET', 'HEAD', 'OPTIONS'),
  fc.constantFrom('POST', 'PUT', 'PATCH', 'DELETE', 'CONNECT', 'TRACE'),
...
describe('gate robustness (Req 7.7)', () => {
  it('fails closed: any token that is not exactly the correct token is UNAUTHENTICATED', () => {
    ...
    fc.assert(
      fc.property(
        fc.oneof(fc.constant<string | undefined>(undefined), fc.string({ maxLength: 40 })).filter((t) => t !== TOKEN),
        methodArb,
        (badToken, method) => {
  ...
  it('a mutating method is never allowed regardless of token', () => {
    ...
    fc.assert(
      fc.property(
        fc.oneof(fc.constant(TOKEN), fc.string({ maxLength: 40 })),
