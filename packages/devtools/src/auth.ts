...
/** The outcome codes the gate can produce. */
export type DevtoolsDecisionCode = 'ALLOWED' | 'UNAUTHENTICATED' | 'READ_ONLY';

/** HTTP status code that corresponds to each decision. */
export const STATUS_FOR_DECISION: Record<DevtoolsDecisionCode, number> = {
  ALLOWED: 200,
  UNAUTHENTICATED: 401,
  READ_ONLY: 403,
};

/** A gate decision. `allowed` is true iff `code === 'ALLOWED'`. */
export interface DevtoolsDecision {
  allowed: boolean;
  code: DevtoolsDecisionCode;
  /** HTTP status to return when denied (200 when allowed). */
  status: number;
  /** Human-readable reason, present when denied. */
  reason?: string;
}

/** An access attempt against the inspected app. */
export interface AccessAttempt {
  /** Raw bearer token presented by the caller (already extracted), if any. */
  token?: string;
  /** HTTP method the tool wants to issue against the inspected app. */
  method: string;
}
...
export class DevtoolsAuthGate {
  ...
  static fromToken(token: string): DevtoolsAuthGate {
  ...
  static fromTokenHash(tokenHash: string): DevtoolsAuthGate {
   ...
   */
  authorize(attempt: AccessAttempt): DevtoolsDecision {
  ...
  authorizeHeader(authorization: string | undefined, method: string): DevtoolsDecision {
