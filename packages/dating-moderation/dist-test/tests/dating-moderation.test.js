...
import { DatingModeration, ModerationToolkit, InMemoryModerationStore, UnknownReportError, } from '../index.js';
...
describe('DatingModeration reporting (R11.4)', () => {
    ...
    it('rejects resolving an unknown report id', async () => {
        const mod = make();
        await assert.rejects(() => mod.resolveReport('mod-1', 'nope', 'banned'), (err) => err instanceof UnknownReportError && err.reportId === 'nope');
    });
});
...
describe('DatingModeration audit log (composes R8.5/R8.7)', () => {
    it('records an append-only event for each block and report', async () => {
        ...
        assert.deepEqual(log.map((e) => e.action), ['report', 'block']);
...
//# sourceMappingURL=dating-moderation.test.js.map