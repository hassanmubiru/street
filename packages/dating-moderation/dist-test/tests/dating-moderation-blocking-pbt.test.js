...
describe('dating-moderation blocking invariant (R11.4)', () => {
    it('canMessage(from,to) is true iff `to` has not blocked `from`, for every pair', async () => {
        await fc.assert(fc.asyncProperty(blocksArb, async (blocks) => {
            ...
            for (const from of USERS) {
                for (const to of USERS) {
                    ...
                    assert.equal(await mod.canMessage(from, to), expectedCanMessage, `canMessage(${from}, ${to}) mismatch`);
                    const expectedBetween = blocked.has(`${from}->${to}`) || blocked.has(`${to}->${from}`);
                    assert.equal(await mod.isBlockedBetween(from, to), expectedBetween, `isBlockedBetween(${from}, ${to}) mismatch`);
                }
            }
        }), { numRuns: NUM_RUNS });
    });
});
//# sourceMappingURL=dating-moderation-blocking-pbt.test.js.map