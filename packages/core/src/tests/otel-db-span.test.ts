...
import type { Span, OtelInstrumentablePool } from '../observability/otel.js';

// ── Helpers ───────────────────────────────────────────────────────────────────

/** A fake queryable pool that records the SQL/params it received. */
function makeFakePool(): OtelInstrumentablePool & { calls: Array<{ sql: string; params?: unknown[] }> } {
...
function makeRejectingPool(err: Error): OtelInstrumentablePool {
  return {
    async query() {
      throw err;
    },
...
function bufferedSpans(tracer: OtelTracer): Span[] {
  return (tracer as unknown as { buffer: Span[] }).buffer;
}
