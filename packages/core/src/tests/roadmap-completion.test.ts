...
describe('gRPC unary round-trip over HTTP/2', () => {
  it('dispatches a unary RPC and returns a response with grpc-status 0', async () => {
    ...
    const result = await new Promise<{ sum: number }>((resolve, reject) => {
      ...
      req.on('end', () => {
        ...
        if (!decoded) { reject(new Error('no frame')); return; }
        resolve(JSON.parse(decoded.payload.toString('utf8')) as { sum: number });
      });
      req.on('error', reject);
