...
import {
  FeatureFlagService,
  FEATURE_FLAGS_MIGRATION_SQL,
} from '../enterprise/feature-flags.js';
import {
  AuditLogger,
  Sensitive,
  ENTERPRISE_AUDIT_MIGRATION_SQL,
} from '../enterprise/audit-logger.js';
import {
  RetainFor,
  Encrypt,
  Classify,
  ComplianceReporter,
  RetentionJob,
} from '../enterprise/data-policy.js';
...
describe('FeatureFlagService', () => {
  ...
  it('percentage rollout is stable for same userId (deterministic hash)', async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 5 }, () => svc.isEnabled('stable-flag', { userId: 'consistent-user-42' })),
    );
...
describe('@Sensitive decorator', () => {
  it('stores field name in metadata', () => {
    class UserEntity {
      ...
      @Sensitive() ssn!: string;
      email!: string;
    }
    const fields = Reflect.getMetadata('street:sensitive', UserEntity) as string[] | undefined;
...
describe('@RetainFor decorator', () => {
  it('stores retention period on property', () => {
    class UserData {
      @RetainFor('90d') logs!: string;
      @RetainFor('1y') profiles!: string;
    }
    const meta = Reflect.getMetadata('street:retention', UserData) as Record<string, string> | undefined;
...
describe('@Encrypt decorator', () => {
  it('stores encrypted field names on class metadata', () => {
    ...
    const fields = Reflect.getMetadata('street:encrypt', PaymentEntity) as string[] | undefined;
...
describe('@Classify decorator', () => {
  it('stores classification level on property', () => {
    class Document {
      ...
      @Classify('public') title!: string;
    }
    const meta = Reflect.getMetadata('street:classify', Document) as Record<string, string> | undefined;
...
describe('ComplianceReporter', () => {
  it('reports fields with all annotations', () => {
    class ReportEntity {
      id!: string;
      @Encrypt() @Classify('confidential') ssn!: string;
      @RetainFor('7y') auditLog!: string;
      name!: string;
    }
  ...
  it('returns empty array for class with no decorated properties', () => {
    class PlainClass { id!: string; name!: string; }
...
describe('RetentionJob', () => {
  ...
  it('run() throws for unsafe table name (SQL injection prevention)', async () => {
    const pool = { async query(): Promise<DbResult> { return { rows: [], rowCount: 0, command: 'DELETE' }; } };
    const job = new RetentionJob(pool);
    await assert.rejects(
      () => job.run([{ table: "users; DROP TABLE users;--", retentionDays: 30 }]),
      /unsafe|invalid|characters/i,
    );
  });
});
