...
function makeCtx(headers: Record<string, string> = {}): StreetContext {
  const responseHeaders: Record<string, string> = {};
  return {
    req: {} as StreetContext['req'],
    res: {} as StreetContext['res'],
    path: '/test',
    method: 'GET',
    params: {},
    query: {},
  ...
  } as unknown as StreetContext & { _responseHeaders: Record<string, string> };
}
...
describe('Logger — Error serialisation (12.4)', () => {
  it('serialises an Error in meta to {name, message, stack}', () => {
    ...
    assert.equal(entries.length, 1);
    const e = entries[0]!;
    const serialised = e['err'] as { name: string; message: string; stack: string };
...
describe('correlationMiddleware (12.6)', () => {
  it('generates a new UUID when no X-Correlation-ID header is present', async () => {
    ...
    const correlationId = ctx.state['correlationId'] as string;
  ...
  it('child logger includes correlationId in entries', async () => {
    ...
    await mw(ctx, async () => {
      const childLog = ctx.state['logger'] as Logger;
  ...
  it('sets X-Correlation-ID response header', async () => {
    ...
    const log = new Logger({ service: 'svc', stream });
    const mw = correlationMiddleware(log);
    const ctx = makeCtx({ 'x-correlation-id': 'resp-header-test' }) as unknown as StreetContext & {
