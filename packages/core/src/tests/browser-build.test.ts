...
function tmpFile(contents: string, name: string): { dir: string; file: string } {
...
describe('browser export conditions', () => {
  it('bundles the browser entry for platform=browser with no Node built-ins', async () => {
    const { dir, file } = tmpFile(
      `import { LruCache, sanitizeString, NotFoundException, STREET_BUILD_TARGET } from ${JSON.stringify(browserEntry)};
       const c = new LruCache({ maxSize: 4 });
       c.set('k', sanitizeString('<b>x</b>'));
       export const out = { v: c.get('k'), t: STREET_BUILD_TARGET, e: new NotFoundException('x').status };`,
      'entry.mjs',
    );
    try {
