...
export class EventStore {
  ...
  async append(
    aggregateId: string,
    events: Omit<DomainEvent, 'aggregateId'>[],
    expectedVersion?: number,
  ): Promise<void> {
    if (events.length === 0) return;

    // Check current version for optimistic concurrency
    const versionResult = await this._pool.query(
      `SELECT COALESCE(MAX(version), -1) AS current_version FROM street_events WHERE aggregate_id = $1`,
      [aggregateId],
    );
    const currentVersion = Number(
      (versionResult.rows[0] as Record<string, unknown>)?.['current_version'] ?? -1,
    );

    if (expectedVersion !== undefined && currentVersion !== expectedVersion) {
      throw new Error(
        `EventStore: optimistic concurrency conflict for aggregate "${aggregateId}". ` +
    ...
    for (const event of events) {
      await this._pool.query(
        `INSERT INTO street_events (aggregate_id, version, type, payload)
         VALUES ($1, $2, $3, $4)`,
        [aggregateId, event.version ?? nextVersion, event.type, JSON.stringify(event.payload)],
      );
      nextVersion++;
    }
  }
  ...
  async load(aggregateId: string, fromVersion = 0): Promise<DomainEvent[]> {
    const result = await this._pool.query(
      `SELECT aggregate_id, version, type, payload
       FROM street_events
