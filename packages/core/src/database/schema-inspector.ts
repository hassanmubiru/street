...
export class SchemaInspector {
  ...
  static readonly _cache = new Map<object, CacheEntry>();
  ...
  static async inspect(
    pool: PgPool | SqlitePool | QueryablePool,
    opts?: { ttlMs?: number },
  ): Promise<DatabaseSchema> {
    const ttlMs = opts?.ttlMs ?? 60_000;
    const now = Date.now();

    const cached = SchemaInspector._cache.get(pool as object);
    ...
    if (SchemaInspector._isSqlitePool(pool)) {
      ...
      schema = await SchemaInspector._inspectPostgres(pool as QueryablePool);
    } else {
      // MySQL or any other pool that satisfies QueryablePool
      schema = await SchemaInspector._inspectMysql(pool as QueryablePool);
    }

    SchemaInspector._cache.set(pool as object, {
      schema,
      expiresAt: now + ttlMs,
    });
  ...
  static invalidateCache(pool: PgPool | SqlitePool | QueryablePool): void {
    SchemaInspector._cache.delete(pool as object);
  }

  // ── Dialect detection ───────────────────────────────────────────────────────

  private static _isSqlitePool(pool: unknown): pool is SqlitePool {
    ...
    return (
      pool !== null &&
      typeof pool === 'object' &&
      (pool as Record<string, unknown>)['constructor'] !== undefined &&
      (pool as { constructor: { name: string } }).constructor.name === 'SqlitePool'
    );
  }

  private static _isPgPool(pool: unknown): boolean {
    return (
      pool !== null &&
      typeof pool === 'object' &&
      (pool as { constructor: { name: string } }).constructor.name === 'PgPool'
    );
  }
   ...
   */
  private static async _inspectPostgres(pool: QueryablePool): Promise<DatabaseSchema> {
   ...
   */
  private static async _inspectMysql(pool: QueryablePool): Promise<DatabaseSchema> {
    const colSql = `
      SELECT
    ...
    const indexAccum = new Map<string, IndexMeta & { tableName: string }>();
    for (const row of idxResult.rows) {
      const tbl = row['table_name'] ?? '';
   ...
   */
  private static async _inspectSqlite(pool: SqlitePool): Promise<DatabaseSchema> {
    ...
    const tables: TableSchema[] = await Promise.all(
      tableNames.map((name) => SchemaInspector._inspectSqliteTable(pool, name)),
    );

    return { tables, inspectedAt: new Date() };
  }

  private static async _inspectSqliteTable(
    pool: SqlitePool,
    tableName: string,
  ): Promise<TableSchema> {
    ...
    const foreignKeys: FkMeta[] = fkResult.rows.map((row) => ({
      column: row['from'] ?? '',
      refTable: row['table'] ?? '',
      refColumn: row['to'] ?? '',
    }));
