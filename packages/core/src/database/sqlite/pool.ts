...
export class SqlitePool {
  ...
  constructor(opts: SqlitePoolOptions) {
    ...
    if (opts.filePath !== ':memory:') {
      process.emitWarning(
        `SqlitePool: filePath "${opts.filePath}" will NOT persist to the real filesystem. ` +
        'SQLite WASM uses an in-process virtual filesystem (Emscripten MEMFS). ' +
        'Use ":memory:" for in-process databases, or node:sqlite (Node.js ≥22.5) for real persistence.',
        'StreetWarning'
      );
    }
  }
