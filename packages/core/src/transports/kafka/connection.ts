...
export class KafkaConnection {
  ...
  private _onData(chunk: Buffer): void {
    ...
    for (;;) {
      ...
      if (cb) { this.pending.delete(correlationId); cb(body); }
    }
  }
  ...
  request(apiKey: number, apiVersion: number, buildBody: (w: KafkaWriter) => void): Promise<KafkaReader> {
    if (!this.socket || this.socket.destroyed) throw new Error('Kafka socket not connected');
    ...
    return new Promise<KafkaReader>((resolve) => {
