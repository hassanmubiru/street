...
export function encodeRecordBatch(records: KafkaRecord[], opts: { producerId?: bigint; producerEpoch?: number; baseSequence?: number } = {}): Buffer {
  ...
  records.forEach((rec, i) => {
    ...
    if (rec.key === null) inner.varint(-1); else { inner.varint(rec.key.length); inner.raw(rec.key); }
    if (rec.value === null) inner.varint(-1); else { inner.varint(rec.value.length); inner.raw(rec.value); }
...
export function decodeRecordBatches(buf: Buffer): KafkaRecord[] {
  ...
  while (offset + 12 <= buf.length) {
    ...
    if (batchLength <= 0 || batchEnd > buf.length) break;
    ...
    const count = r.int32();
    if (magic !== MAGIC) { offset = batchEnd; continue; }
