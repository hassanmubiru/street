...
export class ReplicationCoordinator extends EventEmitter {
   ...
   */
  async reportReplicationLag(gauge: { set(value: number, labels?: Record<string, string>): void }): Promise<void> {
    const primary = this.regions.find((r) => r.isPrimary && r.healthy);
    if (!primary) return;
    const result = await primary.config.pool.query(
      `SELECT application_name AS replica_id,
