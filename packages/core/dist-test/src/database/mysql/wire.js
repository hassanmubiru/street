...
function parseServerGreeting(body) {
    let offset = 0;
...
// ─── HandshakeResponse41 ──────────────────────────────────────────────────────
function buildHandshakeResponse(user, database, authResponse, authPluginName, charset = 0x21 /* utf8_general_ci */) {
...
export class MysqlConnection {
    ...
    streamTarget = null;
    columns = [];
    colCount = 0; // expected column count for result set
    colsReceived = 0; // column defs received so far
    expectEof = false; // waiting for EOF/OK after columns
    ...
    get serverVersion() { return this.greeting?.serverVersion ?? ''; }
    /**
    ...
    _handleAuthPacket(body, firstByte) {
    ...
    _handleQueryPacket(body, firstByte) {
    ...
    _handlePreparePacket(body, firstByte) {
        ...
        if (!pp.stmt.stmtId) {
            if (firstByte !== 0x00) {
                pp.reject(new Error('Unexpected prepare response'));
                return;
            }
...
function buildStmtExecutePacket(stmtId, params) {
    ...
    header.writeUInt32LE(stmtId, off);
    off += 4;
    header[off++] = 0; // flags
    header.writeUInt32LE(1, off);
    off += 4; // iteration-count
    nullBitmap.copy(header, off);
    off += nullBitmapLen;
