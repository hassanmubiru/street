...
export class FaultInjector {
    ...
    shouldFail() {
        const n = this.calls;
        if (this.policy.failEvery && n % this.policy.failEvery === 0)
            return true;
        if (this.policy.failAfter !== undefined && n > this.policy.failAfter)
            return true;
        if (this.policy.errorRate > 0 && this.rand() < this.policy.errorRate)
            return true;
        return false;
    }
    ...
    async run(op) {
        this.calls++;
        if (this.policy.latencyMs > 0)
            await new Promise((r) => setTimeout(r, this.policy.latencyMs));
        if (this.shouldFail()) {
 ...
 */
export function chaosMiddleware(policy = {}) {
 ...
 */
export async function retryWithBackoff(op, opts = {}) {
