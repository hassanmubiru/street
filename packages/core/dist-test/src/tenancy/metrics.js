...
export class TenantUsageAggregator {
    ...
    async aggregate(period) {
        ...
        const result = await this.pool.query(`INSERT INTO street_tenant_daily_stats (tenant_id, date, metrics, created_at)
       ...
       DO UPDATE SET metrics = EXCLUDED.metrics, created_at = NOW()`, [day]);
     ...
     */
    scheduleNightly(scheduler, name = 'tenant-usage-daily-aggregation', expression = '10 0 * * *') {
