...
export function base32Decode(input) {
    ...
    while (end > 0 && upper.charCodeAt(end - 1) === 0x3d /* '=' */)
        end--;
...
export function hotp(secret, counter, opts = {}) {
 ...
 */
export function verifyTotp(secret, code, opts = {}, nowMs = Date.now()) {
...
export class MfaService {
    ...
    async beginEnrollment(userId, accountName) {
        const secretBuf = randomBytes(20);
        const secret = base32Encode(secretBuf);
        const recoveryCodes = generateRecoveryCodes();
        const hashes = recoveryCodes.map(hashCode);
        await this.pool.query(`INSERT INTO street_mfa (user_id, secret_b32, enabled, recovery_hashes)
       VALUES ($1, $2, FALSE, $3)
       ON CONFLICT (user_id) DO UPDATE SET secret_b32 = EXCLUDED.secret_b32, enabled = FALSE, recovery_hashes = EXCLUDED.recovery_hashes`, [userId, secret, JSON.stringify(hashes)]);
    ...
    async confirmEnrollment(userId, code) {
        ...
        if (!verifyTotp(secret, code, this.opts.totp))
            return false;
        await this.pool.query(`UPDATE street_mfa SET enabled = TRUE, confirmed_at = NOW() WHERE user_id = $1`, [userId]);
        return true;
    }
    ...
    async useRecoveryCode(userId, code) {
        ...
        await this.pool.query(`UPDATE street_mfa SET recovery_hashes = $2 WHERE user_id = $1`, [userId, JSON.stringify(remaining)]);
        return true;
    }
    ...
    async _row(userId) {
        const res = await this.pool.query(`SELECT secret_b32, enabled, recovery_hashes FROM street_mfa WHERE user_id = $1 LIMIT 1`, [userId]);
        ...
        return {
            ...
            recovery_hashes: Array.isArray(rh) ? rh : JSON.parse(String(rh ?? '[]')),
        };
    }
}
/**
 ...
 */
export function mfaGuard(service, opts = {}) {
    ...
    return async (ctx, next) => {
        ...
        if (ctx.state[key] === true) {
            await next();
            return;
        }
        if (!(await service.isEnabled(userId))) {
            await next();
            return;
        }
 ...
 */
export async function verifyMfaStepUp(service, userId, code, ctx, opts = {}) {
    if (await service.verify(userId, code)) {
        if (ctx)
            ctx.state[opts.verifiedStateKey ?? 'mfaVerified'] = true;
