...
describe('SchemaInspector — caching', () => {
    ...
    it('cache stores an entry keyed by the pool object', async () => {
        ...
        assert.ok(SchemaInspector._cache.has(pool), 'cache must have an entry for the pool');
    });
    it('returns a fresh schema after TTL has expired (mocked expiresAt)', async () => {
        ...
        const entry = SchemaInspector._cache.get(pool);
    ...
    it('uses the custom TTL passed via opts', async () => {
        ...
        assert.ok(remaining > 0 && remaining <= shortTtl + 50, `expected TTL ~${shortTtl}ms, got ${remaining}ms remaining`);
    });
});
...
describe('SchemaInspector — invalidateCache', () => {
    ...
    it('removes the cache entry after invalidateCache()', async () => {
        await SchemaInspector.inspect(pool);
        assert.ok(SchemaInspector._cache.has(pool), 'entry must exist before invalidation');
        SchemaInspector.invalidateCache(pool);
        assert.ok(!SchemaInspector._cache.has(pool), 'entry must be gone after invalidation');
    });
