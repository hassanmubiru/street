...
describe('caching_sha2_password — fast-auth scramble', () => {
    ...
    it('matches a precomputed known vector for "password"', () => {
        const token = sha2PasswordHash('password', SEED);
        assert.equal(token.toString('hex'), 'f7ab1c623a6e98dceab35e926290e5746a3141116115f4dd8ccca994393eccdd');
    });
    it('matches a precomputed known vector for "secret"', () => {
        const token = sha2PasswordHash('secret', SEED);
        assert.equal(token.toString('hex'), '746ebe205d56a0707acb3e796e834e0dd7b1d61743b26bd5202c7a623230c7c9');
    });
    it('matches independently-precomputed known-answer vectors', () => {
        for (const [pw, hex] of Object.entries(KNOWN_VECTORS)) {
            assert.equal(sha2PasswordHash(pw, SEED).toString('hex'), hex, `scramble mismatch for ${JSON.stringify(pw)}`);
        }
    });
    ...
    it('satisfies the XOR identity: token XOR SHA256(pw) === SHA256(SHA256(SHA256(pw)) || seed)', () => {
        ...
        const a = Buffer.from('5e884898da28047151d0e56f8dc6292773603d0d6aabbdd62a11ef721d1542d8', // SHA256("password")
        'hex');
        const c = Buffer.from('a92354fae0469cadbb63bbfdef56cc5319517c1c0bbe490ba6dd46e6242b8e05', // SHA256(SHA256(SHA256("password")) || SEED)
        'hex');
        const recovered = Buffer.alloc(32);
        for (let i = 0; i < 32; i++)
            recovered[i] = token[i] ^ a[i];
        assert.deepEqual(recovered, c);
    });
    it('is deterministic for identical inputs', () => {
        assert.deepEqual(sha2PasswordHash('password', SEED), sha2PasswordHash('password', SEED));
    });
    it('produces a different token for a different seed', () => {
        ...
        assert.notDeepEqual(sha2PasswordHash('password', SEED), sha2PasswordHash('password', otherSeed));
    });
});
//# sourceMappingURL=mysql-caching-sha2-password.test.js.map