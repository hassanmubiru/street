...
import { EnterpriseConsole, InMemoryConsoleBackend, } from '../enterprise/console/index.js';
...
describe('EnterpriseConsole — lifecycle', () => {
    ...
    it('returns 401 when the token is invalid (Req 6.6)', async () => {
        const { console, backend } = makeConsole();
        const before = backend.snapshot();
        const res = await console.handle(req('POST', '/api/admin/tenants', { authorization: 'Bearer not.a.jwt' }, { name: 'acme' }));
        assert.equal(res.status, 401);
        assert.equal(backend.snapshot(), before);
    });
    it('returns 403 when authenticated but unauthorized (Req 6.7)', async () => {
        ...
        const before = backend.snapshot();
        const res = await console.handle(req('POST', '/api/admin/tenants', bearer(jwt, ['viewer']), { name: 'acme' }));
        assert.equal(res.status, 403);
        assert.deepEqual(res.body.required, ['admin', 'tenant:write']);
        assert.equal(backend.snapshot(), before);
    });
    it('returns 400 identifying the invalid field, state unchanged (Req 6.8)', async () => {
        const { console, backend, jwt } = makeConsole();
        const before = backend.snapshot();
        const res = await console.handle(req('POST', '/api/admin/tenants', bearer(jwt, ['admin']), { name: '' }));
    ...
    it('creates a tenant on a fully valid, authorized request (Req 6.1)', async () => {
        const { console, jwt } = makeConsole();
        const res = await console.handle(req('POST', '/api/admin/tenants', bearer(jwt, ['tenant:write']), { name: 'acme', plan: 'pro' }));
...
describe('EnterpriseConsole — per-area operations', () => {
    ...
    it('rejects an invalid retention value without mutating state (Req 6.8)', async () => {
        const { console, backend, jwt } = makeConsole();
        const before = backend.snapshot();
        const res = await console.handle(req('PUT', '/api/admin/policies/retention', bearer(jwt, ['policy:write']), { entity: 'orders', retentionDays: -5 }));
        assert.equal(res.status, 400);
        assert.equal(res.body.field, 'retentionDays');
        assert.equal(backend.snapshot(), before);
    });
    ...
    it('rejects an audit export with from after to (Req 6.8)', async () => {
        const { console, jwt } = makeConsole();
        const res = await console.handle(req('GET', '/api/admin/compliance/audit-export', bearer(jwt, ['compliance:read']), { from: '2024-03-01', to: '2024-01-01', format: 'csv' }));
        assert.equal(res.status, 400);
...
//# sourceMappingURL=enterprise-console.test.js.map