...
function randomBenign(rng, len) {
    let s = '';
    for (let i = 0; i < len; i++)
        s += BENIGN_CHARS[Math.floor(rng() * BENIGN_CHARS.length)];
    return s;
}
...
describe('Class D preservation — benign sanitizeString is identity', () => {
    it('returns random benign strings unchanged', () => {
        ...
        for (let trial = 0; trial < 1000; trial++) {
            ...
            if (!isBenign(s))
                continue; // skip the rare accidental dangerous token
...
describe('Class D preservation — bounds behave identically', () => {
    ...
    it('MAX_DEPTH: nesting beyond the depth limit collapses to null', () => {
        ...
        const shallow = sanitizeDeep(build(5));
        let cur = shallow;
        for (let i = 0; i < 5; i++)
            cur = cur['next'];
        ...
        assert.equal(node['next'], null, 'the child beyond MAX_DEPTH must collapse to null');
    });
    it('MAX_ARRAY: truncates an over-long array to the cap', () => {
        const arr = new Array(MAX_ARRAY + 50).fill('x');
        const out = sanitizeDeep(arr);
...
//# sourceMappingURL=preservation-class-d-sanitize.test.js.map