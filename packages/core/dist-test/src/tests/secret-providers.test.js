...
import { VaultSecretProvider, AwsSecretsManagerProvider, AzureKeyVaultProvider, GcpSecretManagerProvider, SecretRotationManager, } from '../cloud/secret-providers.js';
class MockServer {
    requests = [];
    server;
    handler = () => { };
    ...
    onRequest(fn) { this.handler = fn; }
    reset() { this.requests.length = 0; }
    async close() { this.server.close(); await once(this.server, 'close').catch(() => undefined); }
}
...
describe('Secret providers (contract tests against mock server)', () => {
    ...
    it('VaultSecretProvider error message redacts the secret value on non-200', async () => {
        ...
        await assert.rejects(provider.get('nope'), (e) => /\[REDACTED\]/.test(e.message) && /403/.test(e.message));
    });
    // ── AWS ──
    ...
    it('AwsSecretsManagerProvider redacts the body in error messages', async () => {
        ...
        await assert.rejects(provider.get('missing'), (e) => /\[REDACTED\]/.test(e.message));
    });
    // ── Azure ──
    ...
    it('GcpSecretManagerProvider redacts the body in error messages', async () => {
        ...
        await assert.rejects(provider.get('x'), (e) => /\[REDACTED\]/.test(e.message));
    });
    // ── Rotation ──
...
//# sourceMappingURL=secret-providers.test.js.map