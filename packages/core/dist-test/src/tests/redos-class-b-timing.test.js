...
function medianMs(fn, samples = 5) {
 ...
 */
function assertLinearTimeBudget(label, run, make) {
    ...
    const report = `${label}: t(N=${NBASE})=${base.toFixed(2)}ms, t(N=${NBIG})=${big.toFixed(2)}ms, ` +
        `observed growth x${observedRatio} (linear≈x${K}), linear budget=${budget.toFixed(2)}ms`;
    assert.ok(big <= budget, `super-linear backtracking detected — ${report} ` +
...
describe('Class B.1 — base32Decode ReDoS (alert #26, auth/mfa.ts:31)', () => {
    it('processes adversarial padding input in linear time', () => {
        ...
        const report = assertLinearTimeBudget('base32Decode("=".repeat(N)+"A")', (input) => {
            try {
                base32Decode(input);
            }
            catch {
...
describe('Class B.3 — parseProto ReDoS (alert #19, grpc/proto-parser.ts:40)', () => {
    it('parses adversarial unterminated block comments in linear time', () => {
        ...
        const report = assertLinearTimeBudget('parseProto("/*a".repeat(N))', (input) => {
            parseProto(input);
        }, (n) => '/*a'.repeat(n));
        console.log(report);
    });
});
//# sourceMappingURL=redos-class-b-timing.test.js.map