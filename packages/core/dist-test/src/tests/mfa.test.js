...
import { hotp, totp, verifyTotp, base32Encode, base32Decode, generateRecoveryCodes, MfaService, mfaGuard, verifyMfaStepUp, MFA_MIGRATION_SQL, } from '../auth/mfa.js';
...
describe('generateRecoveryCodes', () => {
    it('produces unique formatted codes', () => {
        ...
        for (const c of codes)
            assert.match(c, /^[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{2}$/);
    });
});
...
class FakeMfaPool {
    ...
    async query(sql, params = []) {
        ...
        if (s.startsWith('UPDATE STREET_MFA SET RECOVERY_HASHES')) {
            const r = this.store.get(String(params[0]));
            if (r)
                r['recovery_hashes'] = JSON.parse(String(params[1]));
            return { rows: [], rowCount: 1 };
        }
        if (s.startsWith('DELETE FROM STREET_MFA')) {
...
describe('MFA middleware (step-up)', () => {
    async function enabledSvc(userId) {
        const pool = new FakeMfaPool();
        const svc = new MfaService(pool);
    ...
    function ctx(userId) {
        return { user: userId ? { id: userId } : null, state: {}, _status: 0, _body: null, json(b, s = 200) { this._status = s; this._body = b; } };
    }
...
//# sourceMappingURL=mfa.test.js.map