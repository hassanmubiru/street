...
import { createMutualTlsServer, validateClientCert, certificateFingerprint, verifyCertificatePin, clientCertMiddleware, TrustStore, rotateServerCertificate, } from '../security/mtls.js';
let tmp = '';
let opensslOk = false;
...
describe('mTLS — validateClientCert policy', () => {
    const cert = { subject: { CN: 'svc-a' }, raw: Buffer.from('clientcert') };
    ...
    it('rejects an untrusted cert without pinning', () => {
        ...
        assert.equal(r.reason, 'untrusted_client_certificate');
    });
...
describe('mTLS — real handshake (requires openssl)', () => {
    it('accepts a client presenting a CA-signed cert and rejects one without', async (t) => {
        ...
        const f = (n) => join(tmp, n);
        const server = createMutualTlsServer({ cert: readFileSync(f('server.crt')), key: readFileSync(f('server.key')), ca: readFileSync(f('ca.crt')), rejectUnauthorized: true }, (req, res) => {
            // Apply the middleware-equivalent validation.
            void clientCertMiddleware({ allowedCommonNames: ['street-client'] })({ req, state: {} }, async () => undefined).then(() => { res.writeHead(200); res.end('authorized'); })
...
describe('mTLS — TrustStore', () => {
    ...
    it('validates via configured pins; rotate() swaps the whole set', () => {
        const cert = { subject: { CN: 'svc' }, raw: Buffer.from('rawcert') };
        const pin = certificateFingerprint(cert.raw);
...
describe('mTLS — live certificate rotation (requires openssl)', () => {
    it('rotateServerCertificate swaps the trusted client CA without restart', async (t) => {
        ...
        const f = (n) => join(tmp, n);
        const server = createMutualTlsServer({ cert: readFileSync(f('server.crt')), key: readFileSync(f('server.key')), ca: readFileSync(f('ca.crt')), rejectUnauthorized: true }, (_req, res) => { res.writeHead(200); res.end('ok'); });
        server.listen(0, '127.0.0.1');
        await once(server, 'listening');
        const port = server.address().port;
        ...
        const call = (clientCrt, clientKey) => new Promise((resolve) => {
...
//# sourceMappingURL=mtls.test.js.map