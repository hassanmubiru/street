...
import { AmqpWriter, AmqpReader, buildFrame, buildMethodFrame, buildHeaderFrame, buildBodyFrame, buildHeartbeat, FrameDecoder, readMethodHeader, FRAME_METHOD, FRAME_HEADER, FRAME_BODY, FRAME_HEARTBEAT, FRAME_END, PROTOCOL_HEADER, } from '../transports/rabbitmq/codec.js';
describe('AMQP codec — field encoding', () => {
    it('round-trips octet, shortUint, longUint, longLong', () => {
        const buf = new AmqpWriter().octet(7).shortUint(258).longUint(70000).longLong(9000000000n).build();
        const r = new AmqpReader(buf);
        assert.equal(r.octet(), 7);
        assert.equal(r.shortUint(), 258);
        assert.equal(r.longUint(), 70000);
        assert.equal(r.longLong(), 9000000000n);
    });
...
//# sourceMappingURL=rabbitmq-codec.test.js.map