    ...
    enqueue(target, event, data) {
        ...
        validateWebhookUrl(target.url, this.allowedHosts)
            ...
            if (!this._warnedUrls.has(key)) {
                ...
                console.error(`[webhook] URL validation failed, dropping event "${event}": ${msg}\n` +
 ...
 */
export function buildRequestOptions(url, contentLength, signature, timeoutMs, tls) {
...
function sendRequest(url, body, signature, timeoutMs, tls) {
    return new Promise((resolve, reject) => {
        ...
        const req = requester(buildRequestOptions(url, bodyBuf.length, signature, timeoutMs, tls), (res) => {
