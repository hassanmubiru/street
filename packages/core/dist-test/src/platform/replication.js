...
export class ReplicationCoordinator extends EventEmitter {
     ...
     */
    getReadPool(preferredRegion) {
        if (preferredRegion) {
            const preferred = this.regions.find((r) => r.config.name === preferredRegion && r.healthy);
            if (preferred)
                return preferred.config.pool;
        }
...
// ---------------------------------------------------------------------------
export function preferredRegionMiddleware(coordinator) {
    return async (ctx, next) => {
        const preferredRegion = ctx.headers['x-preferred-region'];
        try {
