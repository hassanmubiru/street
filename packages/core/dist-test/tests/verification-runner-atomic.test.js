...
const blockedReasonArb = fc.record({
...
const validArtifactArb = fc
    .record({
    capabilityId: capabilityIdArb,
    status: fc.constantFrom('VERIFIED', 'PARTIAL', 'BLOCKED', 'NOT_IMPLEMENTED'),
    ...
    if (r.status === 'BLOCKED')
        base.blockedReason = r.blockedReason;
    return base;
});
...
async function assertNoTempLeftover(dir) {
...
function assertCompleteAndValid(artifact) {
...
describe('Property 3a: every artifact CommandRunner.run() produces is complete and valid (Req 1.7)', () => {
    it('writes a schema-valid artifact with all required fields for any run outcome', async () => {
        await fc.assert(fc.asyncProperty(capabilityIdArb, fc.integer({ min: 0, max: 5 }), fc.record({
            sourceCode: fc.boolean(),
            passingTests: fc.boolean(),
            documentation: fc.boolean(),
            artifact: fc.boolean(),
        }, { requiredKeys: [] }), fc.boolean(), async (capabilityId, exitCode, evidenceHints, withPrereq) => {
...
describe('Property 3b: writeArtifactAtomic persists complete, valid artifacts (Req 1.7)', () => {
    it('round-trips any schema-valid artifact byte-for-byte and leaves no temp file', async () => {
        await fc.assert(fc.asyncProperty(validArtifactArb, async (artifact) => {
            await withTempDir(async (dir) => {
...
describe('Property 3c: writeArtifactAtomic is atomic at every induced failure point (Req 1.11)', () => {
    it('throws and leaves no partial artifact / no leftover temp file when the write cannot complete', async () => {
        await fc.assert(fc.asyncProperty(validArtifactArb, 
        ...
        fc.constantFrom('rename-onto-dir', 'parent-is-file'), async (artifact, mode) => {
            await withTempDir(async (dir) => {
                let target;
                ...
                await assert.rejects(CommandRunner.writeArtifactAtomic(target, artifact), 'expected writeArtifactAtomic to reject at the induced failure point');
...
describe('Property 3d: CommandRunner.run() surfaces write failure and leaves no partial artifact (Req 1.11)', () => {
    it('throws a capability-named error and leaves no leftover temp file when the artifact cannot be written', async () => {
        await fc.assert(fc.asyncProperty(capabilityIdArb, async (capabilityId) => {
            await withTempDir(async (dir) => {
                const runner = new CommandRunner();
                ...
                await assert.rejects(runner.run({
                    ...
                    assert.ok(err.message.includes(capabilityId), 'error should name the affected capability');
                    return true;
                });
                await assertNoTempLeftover(dir);
            });
...
//# sourceMappingURL=verification-runner-atomic.test.js.map