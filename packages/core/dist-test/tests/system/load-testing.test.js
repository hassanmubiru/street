...
describe('HTTP Server — concurrent load testing', () => {
    ...
    it(`handles ${CONCURRENCY} concurrent POST requests with body parsing`, async () => {
        const results = await Promise.all(Array.from({ length: CONCURRENCY }, async (i) => {
            ...
            const echoed = parsed.echoed;
            assert.equal(typeof echoed, 'string', 'echoed should be a string');
            assert.ok(echoed.length > 100, `echoed too short: ${echoed.length}`);
            return res;
        }));
...
describe('Router — throughput testing', () => {
    it('matches and dispatches thousands of routes', async () => {
        ...
        const { createContext } = await import('../../src/core/context.js');
        const fakeReq = { method: 'GET', url: '/users/abc/posts/def', headers: {}, socket: { remoteAddress: '' } };
        const fakeRes = { writeHead: () => undefined, write: () => true, end: () => undefined, setHeader: () => undefined, writableEnded: false, once: () => fakeRes, on: () => fakeRes };
    ...
    it('creates and closes many SSE connections', async () => {
        ...
        for (let i = 0; i < 100; i++) {
            const fakeRes = {
                ...
                writableEnded: false,
                once: (_event, cb) => { if (_event === 'close')
                    setTimeout(cb, 1); },
                on: () => fakeRes,
                socket: { once: (_event, cb) => { if (_event === 'end')
                        setTimeout(cb, 1); } },
            };
