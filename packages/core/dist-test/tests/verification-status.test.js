...
import { classify, } from '../src/verification/status.js';
...
const blockedArb = fc.oneof(fc.constant(undefined), fc.constant(null), fc.record({
...
const exitCodeArb = fc.oneof(fc.constant(0), fc.integer({ min: 1, max: 255 }), fc.integer({ min: -255, max: -1 }));
...
function expectedStatus(input) {
    if (!input.hasSourceCode)
        return 'NOT_IMPLEMENTED'; // Req 1.6
    if (input.timedOut || input.blocked != null)
        return 'BLOCKED'; // Req 1.5 / 1.10
    const e = input.evidence;
    const allEvidence = e.sourceCode && e.passingTests && e.documentation && e.artifact;
    if (allEvidence && input.commandExitCode === 0)
        return 'VERIFIED'; // Req 1.3
    return 'PARTIAL'; // Req 1.4 / 1.9
}
const FOUR_STATUSES = new Set([
    'VERIFIED',
    'PARTIAL',
    'BLOCKED',
    'NOT_IMPLEMENTED',
...
describe('Property 1: status classification is deterministic and honors precedence', () => {
    it('always returns exactly one of the four statuses (Req 1.1)', () => {
        fc.assert(fc.property(classifyInputArb, (input) => {
    ...
    it('NOT_IMPLEMENTED wins when there is no source code, regardless of all else (Req 1.6)', () => {
        fc.assert(fc.property(classifyInputArb, (input) => {
            const noSource = { ...input, hasSourceCode: false };
    ...
    it('BLOCKED wins over VERIFIED/PARTIAL whenever timed out or blocked, given source (Req 1.5/1.10)', () => {
        fc.assert(fc.property(classifyInputArb, fc.boolean(), (input, useTimeout) => {
            const blockedInput = {
                ...input,
                hasSourceCode: true,
                ...(useTimeout
    ...
    it('a non-zero exit code never yields VERIFIED; with source/not-blocked it is PARTIAL (Req 1.9)', () => {
        fc.assert(fc.property(evidenceArb, fc.oneof(fc.integer({ min: 1, max: 255 }), fc.integer({ min: -255, max: -1 })), (evidence, nonZeroExit) => {
            const input = {
                hasSourceCode: true,
                evidence,
                blocked: null,
...
//# sourceMappingURL=verification-status.test.js.map