...
describe('CommandRunner timeout kills a sleeping command (Req 1.10)', () => {
    ...
    it('terminates `sleep 5` at a small injected timeout and records BLOCKED-with-timeout', async () => {
        ...
        assert.ok(elapsedMs < SLEEP_SECONDS * 1000, `expected the run to finish before the ${SLEEP_SECONDS}s sleep, took ${elapsedMs}ms`);
        ...
        assert.ok(durationMs < SLEEP_SECONDS * 1000, `expected durationMs < ${SLEEP_SECONDS * 1000}, got ${durationMs}`);
...
//# sourceMappingURL=verification-runner-unit.test.js.map