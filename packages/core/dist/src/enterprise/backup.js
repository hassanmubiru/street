...
export class BackupService {
    ...
    async backup() {
        ...
        await this.pool.query(`INSERT INTO street_backups (id, size_bytes, duration_ms, checksum, storage_key)
       VALUES ($1, $2, $3, $4, $5)`, [backupId, totalBytes, durationMs, checksum, storageKey]);
        return backupId;
    }
    /**
     * Restores from a backup, verifying SHA-256 checksum before applying.
     */
    async restore(backupId, targetPool) {
        const dest = targetPool ?? this.pool;
        const result = await this.pool.query('SELECT checksum, storage_key FROM street_backups WHERE id = $1', [backupId]);
        if (result.rows.length === 0) {
            throw new Error(`Backup not found: ${backupId}`);
        }
        const { checksum, storage_key } = result.rows[0];
        ...
        if (actualChecksum !== checksum) {
            throw new Error(`Backup checksum mismatch for ${backupId}.\nExpected: ${checksum}\nActual:   ${actualChecksum}`);
        }
...
function _rowsToInsertSql(table, rows) {
    if (rows.length === 0)
        return '';
