...
async function brokerAvailable() {
...
async function ensureTopic(client, topic, minPartitions = 1) {
    const deadline = Date.now() + 15000;
    for (;;) {
        const meta = await client.metadata([topic]);
        const tm = meta.topics.find((t) => t.name === topic);
        if (tm && tm.error === 0 && tm.partitions.length >= minPartitions)
            return;
        if (Date.now() > deadline)
            throw new Error(`topic ${topic} not ready`);
...
describe('Kafka transport (integration)', () => {
    ...
    before(async () => {
        ...
        if (available)
            client = new KafkaClient(clientOpts());
    });
    after(async () => {
        if (available && client)
            client.close();
    ...
    it('batches multiple records and consumes them in order', async (t) => {
        ...
        for (const v of sent)
            assert.ok(got.includes(v), `missing ${v} in ${JSON.stringify(got)}`);
        // ordering within a partition is preserved
        const idxs = sent.map((v) => got.indexOf(v));
        for (let i = 1; i < idxs.length; i++)
            assert.ok(idxs[i] > idxs[i - 1], 'ordering not preserved');
        await producer.close();
    });
    it('distributes records across partitions (round-robin)', async (t) => {
        ...
        for (const p of partitions)
            starts.set(p, await client.listOffset(topic, p, -1n));
        const producer = new KafkaProducer(client);
        const N = partitions.length * 4;
        const sends = [];
        for (let i = 0; i < N; i++)
            sends.push(producer.send(topic, { key: null, value: Buffer.from(`p-${i}`) }));
        await Promise.all(sends);
        await producer.flush();
    ...
    it('runs a KafkaConsumer poll loop with auto-commit and stops gracefully', async (t) => {
        ...
        for (const p of payloads)
            await producer.send(topic, { key: null, value: Buffer.from(p) }, 0);
        await producer.flush();
        const received = [];
        const consumer = new KafkaConsumer(client, { groupId: group, topic, partitions: [0], fromBeginning: true });
        await consumer.run(async (msg) => { if (msg.value)
            received.push(msg.value.toString('utf8')); });
