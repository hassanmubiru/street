...
import { AmqpConnection, RabbitMqConnectionManager, RabbitMqPublisher, RabbitMqConsumer, } from '../../transports/rabbitmq/index.js';
...
describe('RabbitMQ transport (integration)', () => {
    ...
    before(async () => {
        ...
        if (available)
            manager = new RabbitMqConnectionManager({ host: HOST, port: PORT, exchange });
    });
    after(async () => {
    ...
    it('publishes (with confirms) and consumes a message', async (t) => {
        if (!available) {
            t.skip('RabbitMQ broker not reachable');
            return;
        }
...
//# sourceMappingURL=rabbitmq.integration.test.js.map