...
import { MigrationDiffer, } from '../database/migrations.js';
...
function makeEntity(spec) {
    ...
    if (spec.indexes)
        Reflect.defineMetadata('street:indexes', spec.indexes, Entity);
    if (spec.primaryKey)
        Reflect.defineMetadata('street:primaryKey', spec.primaryKey, Entity);
    return Entity;
}
...
describe('MigrationDiffer — SQL injection defense', () => {
    ...
    it('rejects an entity table name containing unsafe characters', async () => {
        ...
        await assert.rejects(() => MigrationDiffer.diff(pool, [evil]), /Unsafe table name/);
    });
    it('rejects an entity column name containing unsafe characters', async () => {
        ...
        await assert.rejects(() => MigrationDiffer.diff(pool, [evil]), /Unsafe column name/);
    });
});
    ...
    it('additive-only entity definitions never yield destructive statements', async () => {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            const extraDefaulted = (i % 2 === 0)
                ? [{ name: `flag_${i}`, type: 'INTEGER', nullable: false, default: '0' }]
                : [];
            const indexes = (i % 3 === 0)
            ...
            const diff = await MigrationDiffer.diff(pool, [users]);
            assert.deepEqual(diff.destructive, [], `iteration ${i} unexpectedly produced destructive statements: ${diff.destructive.join(' | ')}`);
            SchemaInspector.invalidateCache(pool);
        }
    });
});
//# sourceMappingURL=migration-diff.test.js.map