...
import { sessionRevocationMiddleware, AUDIT_LOG_MIGRATION_SQL, auditLoginSuccess, auditLoginFailure, auditLogout, auditTokenRefresh, auditSessionRevoked, auditPermissionDenied, } from '../auth/session-store.js';
...
describe('Session revocation middleware (Task 21.6) — revoked session returns 401', () => {
    it('passes an active session through, then returns 401 after the session is revoked', async () => {
        ...
        let nextCalls = 0;
        await mw({ state: { sessionId } }, async () => { nextCalls += 1; });
        ...
        await assert.rejects(() => mw({ state: { sessionId } }, async () => { nextCalls += 1; }), (err) => {
            assert.ok(err instanceof UnauthorizedException, 'should throw UnauthorizedException');
            assert.equal(err.status, 401, 'status must be 401');
            return true;
        });
    ...
    it('treats an unknown session id as revoked (DB fallback) and returns 401', async () => {
        ...
        const store = new StreetSessionStore(pool);
        const mw = sessionRevocationMiddleware(store);
        await assert.rejects(() => mw({ state: { sessionId: 'never-created-session' } }, async () => { }), (err) => {
            assert.ok(err instanceof UnauthorizedException);
            assert.equal(err.status, 401);
            return true;
        });
    ...
    it('skips the revocation check when no session id is present in ctx.state', async () => {
        ...
        const mw = sessionRevocationMiddleware(store);
        let nextCalled = false;
        await mw({ state: {} }, async () => { nextCalled = true; });
...
describe('AuditWriter (Task 21.6) — writes an entry for each of the 6 event types', () => {
    ...
    it('per-event helper functions each write their corresponding event type', async () => {
        ...
        const recorded = pool.events.map((e) => e.event).sort();
        assert.deepEqual(recorded, [...ALL_EVENTS].sort(), 'each helper writes exactly one entry of its event type');
    });
});
...
describe('AuditWriter (Task 21.6) — a failed audit write causes transaction rollback', () => {
    it('rejects and rolls back its own transaction (nothing committed) when the INSERT fails', async () => {
        ...
        const audit = new AuditWriter(pool);
        await assert.rejects(() => audit.write({ event: 'login_success', actorId: 'u1' }), /simulated write failure/);
    ...
    it('rolls back the originating request transaction so business writes are discarded', async () => {
        ...
        assert.equal(pool.committed.find((c) => c.sql.toUpperCase().includes('INSERT INTO ORDERS')), undefined, 'the business write must be rolled back when the audit write fails');
...
void describe('street_audit_log append-only enforcement (Task 21.6, requires Postgres)', () => {
    ...
    after(async () => {
        if (pool) {
            ...
            try {
                await pool.query('DROP TABLE IF EXISTS street_audit_log');
            }
            catch { /* ignore */ }
            try {
                await pool.close();
    ...
    it('rejects DELETE of an audit row (append-only trigger)', async () => {
        if (!pgAvailable || !pool)
            return;
        const inserted = await pool.query(`INSERT INTO street_audit_log (event, actor_id) VALUES ($1, $2) RETURNING id`, ['login_success', 'actor-del']);
        const id = inserted.rows[0]?.['id'];
        assert.ok(id, 'insert should return the generated id');
        await assert.rejects(() => pool.query('DELETE FROM street_audit_log WHERE id = $1', [id]), /append-only/i, 'DELETE must be blocked by the append-only trigger');
    ...
    it('rejects UPDATE of an audit row (append-only trigger)', async () => {
        if (!pgAvailable || !pool)
            return;
        const inserted = await pool.query(`INSERT INTO street_audit_log (event, actor_id) VALUES ($1, $2) RETURNING id`, ['logout', 'actor-upd']);
        const id = inserted.rows[0]?.['id'];
        assert.ok(id, 'insert should return the generated id');
        await assert.rejects(() => pool.query(`UPDATE street_audit_log SET event = 'tampered' WHERE id = $1`, [id]), /append-only/i, 'UPDATE must be blocked by the append-only trigger');
    ...
    it('allows INSERT (append) of audit rows', async () => {
        if (!pgAvailable || !pool)
            return;
        await assert.doesNotReject(() => pool.query(`INSERT INTO street_audit_log (event, actor_id) VALUES ($1, $2)`, ['token_refresh', 'actor-ins']), 'appending new audit rows must be permitted');
    });
});
