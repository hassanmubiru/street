...
import { PROTOCOL_HEADER, FRAME_METHOD, FRAME_HEADER, FRAME_BODY, FRAME_HEARTBEAT, AmqpWriter, FrameDecoder, buildMethodFrame, buildHeaderFrame, buildBodyFrame, buildHeartbeat, readMethodHeader, } from './codec.js';
...
export class AmqpConnection extends EventEmitter {
    ...
    _key(classId, methodId) { return `${classId}.${methodId}`; }
    _send(frame) {
        if (!this.socket || this.socket.destroyed)
            throw new Error('AMQP socket not connected');
        this.socket.write(frame);
    }
    _rpc(sendFrame, expectClass, expectMethod) {
    ...
    async connect() {
        ...
        await new Promise((resolve, reject) => {
            ...
            const onError = (err) => { clearTimeout(to); if (!handshakeDone)
                reject(err);
            else
                this.emit('error', err); };
            ...
            this.once('_handshakeError', (e) => { clearTimeout(to); reject(e); });
        });
    }
    _onData(chunk) {
    ...
    _handleFrame(frame) {
        ...
        if (classId === 10 && methodId === 30) {
            this._sendTuneOkAndOpen(reader);
            return;
        } // Connection.Tune
        if (classId === 10 && methodId === 41) {
            this._afterConnectionOpen();
            return;
        } // Connection.Open-Ok
        if (classId === 10 && methodId === 50) {
            this._handleServerClose(reader);
            return;
        } // Connection.Close
        // Async deliveries / confirms
        if (classId === 60 && methodId === 60) {
            this._beginDelivery(reader);
            return;
        } // Basic.Deliver
        if (classId === 60 && methodId === 80) {
            this._handleConfirm(reader, false);
            return;
        } // Basic.Ack
        if (classId === 60 && methodId === 120) {
            this._handleConfirm(reader, true);
            return;
        } // Basic.Nack
        // Synchronous replies
    ...
    _afterConnectionOpen() {
        ...
        this._rpc(buildMethodFrame(CH, 20, 10, new AmqpWriter().shortStr('').build()), 20, 11)
            ...
            .catch((e) => this.emit('_handshakeError', e));
    }
    _startHeartbeat() {
        if (this.opts.heartbeatSeconds <= 0)
            return;
    ...
    async declareExchange(name, type, opts = {}) {
        const args = new AmqpWriter()
    ...
    async declareQueue(name, opts = {}) {
        const tableArgs = {};
        if (opts.deadLetterExchange)
            tableArgs['x-dead-letter-exchange'] = opts.deadLetterExchange;
        if (typeof opts.messageTtlMs === 'number')
            tableArgs['x-message-ttl'] = opts.messageTtlMs;
        const args = new AmqpWriter()
            .shortUint(0)
            .shortStr(name)
     ...
     */
    async publish(exchange, routingKey, body, opts = {}) {
        ...
        if (opts.persistent !== false)
            props['deliveryMode'] = 2; // persistent by default
        ...
        const confirm = this.confirmEnabled
            ? new Promise((resolve, reject) => {
                ...
                this.confirmWaiters.set(`rej:${tag}`, (() => reject(new Error('publish nacked'))));
            })
            : Promise.resolve();
        ...
        if (body.length > 0)
            this._send(buildBodyFrame(CH, body)); // body
        await confirm;
    }
    _handleConfirm(reader, nack) {
    ...
    _handleBody(frame) {
        ...
        if (have >= this.pendingDelivery.bodySize)
            this._finishDelivery();
    }
    _finishDelivery() {
        if (!this.pendingDelivery)
        ...
        const message = { ...msg, body: Buffer.concat(chunks) };
