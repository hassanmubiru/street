...
export function encryptSecret(plaintext, kek) {
...
export function decryptSecret(blob, kek) {
...
export function loadConfig(instance, kek) {
    const fields = getConfigFields(instance.constructor);
    for (const field of fields) {
        ...
        if (field.encrypted) {
            if (!kek)
                throw new Error(`KEK required to decrypt ${field.envKey}`);
