...
describe('QueryBuilder — select + where + limit (postgres)', () => {
    ...
    it('handles multiple WHERE conditions', () => {
        const { sql, params } = new QueryBuilder(SqlDialect.postgres)
            ...
            .limit(5)
            .build();
        assert.equal(sql, 'SELECT id, name FROM users WHERE age >= $1 AND status = $2 LIMIT 5');
    ...
    it('emits OFFSET clause', () => {
        const { sql } = new QueryBuilder(SqlDialect.postgres)
            .select('id')
            .from('users')
            .limit(10)
            .offset(20)
...
describe('QueryBuilder — join + subquery', () => {
    it('builds INNER JOIN with ON condition', () => {
        const { sql, params } = new QueryBuilder(SqlDialect.postgres)
            ...
            .build();
        assert.equal(sql, 'SELECT id, name FROM users INNER JOIN orders ON users.id = orders.user_id');
        assert.deepEqual(params, []);
    });
    it('builds LEFT JOIN', () => {
        const { sql } = new QueryBuilder(SqlDialect.postgres)
    ...
    it('uses subquery as FROM source (postgres)', () => {
        const inner = new QueryBuilder(SqlDialect.postgres)
...
describe('QueryBuilder — idempotent build', () => {
    it('calling build() twice returns identical sql', () => {
        const qb = new QueryBuilder(SqlDialect.postgres)
            .select('id', 'name')
            .from('users')
            .where('age', '>=', 18)
            .where('status', '=', 'active')
...
describe('QueryBuilder — placeholder count matches params array', () => {
    it('postgres: $n placeholder count equals params.length', () => {
        ...
        assert.equal(matches.length, params.length, `placeholder count ${matches.length} ≠ params.length ${params.length}; sql="${sql}"`);
    });
    it('mysql: ? placeholder count equals params.length', () => {
        const { sql, params } = new QueryBuilder(SqlDialect.mysql)
            .select('id', 'name')
            .from('users')
            .where('age', '>=', 25)
    ...
    it('sqlite: ? placeholder count equals params.length', () => {
        const { sql, params } = new QueryBuilder(SqlDialect.sqlite)
            .select('id', 'total')
            .from('orders')
            .where('total', '>', 100)
            .where('user_id', '=', 7)
...
describe('QueryBuilder — dialect-specific placeholder style', () => {
    const buildWithDialect = (d) => new QueryBuilder(d)
        .select('id')
        .from('users')
