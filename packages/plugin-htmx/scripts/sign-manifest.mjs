...
// (Ed25519). Invoked ONLY by `prepublishOnly` (during publish) and the publish
// workflow, never by `build`, so a local/CI `npm run build` never mutates the
// committed signed manifest. STREET_PLUGIN_SIGNING_KEY (PKCS#8 PEM) is REQUIRED —
// the script fails loudly if absent, so an unsigned/ephemerally-signed package
// can never be published. Pure Node — no third-party dependencies.
