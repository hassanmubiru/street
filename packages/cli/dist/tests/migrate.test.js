...
// ── migrate:diff — buildDiffSql gating logic (no DB required) ──────────
void describe('migrate:diff buildDiffSql', () => {
    const FIXED_TS = '2025-01-01T00:00:00.000Z';
    void it('returns null when there are no changes at all', () => {
        const sql = buildDiffSql({ safe: [], destructive: [] }, false, FIXED_TS);
        assert.equal(sql, null);
    });
    void it('returns null when only destructive changes exist and not confirmed (no spurious file)', () => {
        const sql = buildDiffSql({ safe: [], destructive: ['ALTER TABLE users DROP COLUMN email;'] }, false, FIXED_TS);
        assert.equal(sql, null, 'Must not produce a header-only file when nothing is written');
    });
    void it('writes safe statements but excludes destructive when not confirmed', () => {
        const sql = buildDiffSql({
            safe: ['ALTER TABLE users ADD COLUMN bio TEXT;'],
            destructive: ['ALTER TABLE users DROP COLUMN email;'],
        }, false, FIXED_TS);
        assert.ok(sql !== null);
        assert.ok(sql.includes('ALTER TABLE users ADD COLUMN bio TEXT;'), 'safe statement should be present');
        assert.ok(!sql.includes('DROP COLUMN email'), 'destructive statement must be excluded when not confirmed');
        assert.ok(sql.includes('-- Safe changes (additive)'));
        assert.ok(!sql.includes('-- Destructive changes'));
    });
    void it('includes destructive statements when confirmed', () => {
        const sql = buildDiffSql({
            safe: ['ALTER TABLE users ADD COLUMN bio TEXT;'],
            destructive: ['ALTER TABLE users DROP COLUMN email;'],
        }, true, FIXED_TS);
        assert.ok(sql !== null);
        assert.ok(sql.includes('ALTER TABLE users ADD COLUMN bio TEXT;'));
        assert.ok(sql.includes('ALTER TABLE users DROP COLUMN email;'), 'destructive statement must be written when confirmed');
        assert.ok(sql.includes('-- Destructive changes (column removal)'));
    });
    void it('writes only-destructive changes to a file when confirmed', () => {
        const sql = buildDiffSql({ safe: [], destructive: ['ALTER TABLE users DROP COLUMN email;'] }, true, FIXED_TS);
        assert.ok(sql !== null, 'confirmed destructive-only diff should produce a file');
        assert.ok(sql.includes('ALTER TABLE users DROP COLUMN email;'));
        assert.ok(!sql.includes('-- Safe changes (additive)'), 'no safe section when there are no safe statements');
    });
    void it('embeds the generation timestamp in the header', () => {
        const sql = buildDiffSql({ safe: ['CREATE TABLE posts (id UUID PRIMARY KEY);'], destructive: [] }, false, FIXED_TS);
        assert.ok(sql !== null);
        assert.ok(sql.includes(`-- Generated by street migrate:diff at ${FIXED_TS}`));
    });
});
