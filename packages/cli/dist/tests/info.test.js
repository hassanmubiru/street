...
void describe('InfoCommand', () => {
    ...
    void it('reads Node.js version from process.version', async () => {
        ...
        const allOutput = output.logs.join('\n');
        assert.ok(allOutput.includes(process.version), `Expected Node.js version ${process.version} in output:\n${allOutput}`);
    });
    ...
    void it('falls back to (not found) when TypeScript is not installed', async () => {
        ...
        try {
            ...
            const allOutput = output.logs.join('\n');
            assert.ok(allOutput.includes('(not found)'), `Expected '(not found)' when TypeScript is absent:\n${allOutput}`);
        }
        finally {
...
void describe('DoctorCommand', () => {
    ...
    void it('detects Node.js >= 20 as a pass (no exit code set by Node check)', async () => {
        ...
        const allOutput = output.logs.join('\n');
        assert.ok(allOutput.includes('v20.0.0') && allOutput.includes('✓'), `Expected v20.0.0 ✓ in doctor output:\n${allOutput}`);
    });
    void it('detects missing TypeScript as a failure', async () => {
        ...
        const allOutput = output.logs.join('\n');
        assert.ok(allOutput.includes('not found') || allOutput.includes('install TypeScript'), `Expected TypeScript missing message in output:\n${allOutput}`);
    ...
    void it('skips env var check when no .env.example is present', async () => {
        ...
        const allOutput = output.logs.join('\n');
        assert.ok(allOutput.includes('skipped') || allOutput.includes('no .env.example'), `Expected 'skipped' note for missing .env.example:\n${allOutput}`);
    });
...
class MockAuditCommand extends AuditCommand {
    mockJson;
    mockCode;
    constructor(mockJson, mockCode = 0) {
        super();
...
void describe('AuditCommand', () => {
    ...
    void it('shows "No vulnerabilities found" when audit JSON has no vulnerabilities', async () => {
        ...
        const allOutput = output.logs.join('\n');
        assert.ok(allOutput.includes('No vulnerabilities found'), `Expected 'No vulnerabilities found' in output:\n${allOutput}`);
