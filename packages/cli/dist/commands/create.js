...
export class CreateCommand {
    ...
    async scaffoldProject(targetDir, projectName) {
        ...
        // package.json
        await writeFile(join(targetDir, 'package.json'), this.renderPackageJson(projectName), 'utf8');
        // street.config.ts
        await writeFile(join(targetDir, 'street.config.ts'), this.renderStreetConfig(projectName), 'utf8');
        // src/main.ts
        await writeFile(join(targetDir, 'src/main.ts'), this.renderMainTs(), 'utf8');
        // src/controllers/example.controller.ts
        await writeFile(join(targetDir, 'src/controllers/example.controller.ts'), this.renderExampleController(), 'utf8');
        // src/controllers/health.controller.ts
        await writeFile(join(targetDir, 'src/controllers/health.controller.ts'), this.renderHealthController(), 'utf8');
        // src/services/example.service.ts
        await writeFile(join(targetDir, 'src/services/example.service.ts'), this.renderExampleService(), 'utf8');
        // src/repositories/example.repository.ts
        await writeFile(join(targetDir, 'src/repositories/example.repository.ts'), this.renderExampleRepository(), 'utf8');
        // src/middleware/auth.ts
        await writeFile(join(targetDir, 'src/middleware/auth.ts'), this.renderAuthMiddleware(), 'utf8');
        // src/gateways/chat.gateway.ts
        await writeFile(join(targetDir, 'src/gateways/chat.gateway.ts'), this.renderChatGateway(), 'utf8');
        // tsconfig.json
        await writeFile(join(targetDir, 'tsconfig.json'), this.renderTsconfig(), 'utf8');
        // Dockerfile
        await writeFile(join(targetDir, 'Dockerfile'), this.renderDockerfile(), 'utf8');
        // docker-compose.yml
        await writeFile(join(targetDir, 'docker-compose.yml'), this.renderDockerCompose(), 'utf8');
        // docker-init/001_enable_pgcrypto.sql
        await writeFile(join(targetDir, 'docker-init/001_enable_pgcrypto.sql'), 'CREATE EXTENSION IF NOT EXISTS pgcrypto;\n', 'utf8');
        // .env.example
        await writeFile(join(targetDir, '.env.example'), this.renderEnvExample(), 'utf8');
        // .gitignore
        ...
        // README.md
        await writeFile(join(targetDir, 'README.md'), this.renderReadme(projectName), 'utf8');
    }
    // ─── Helpers ──────────────────────────────────────────────────────────
    ...
    renderStreetConfig(_projectName) {
