 ...
 */
export function renderJobsTable(snapshot) {
    const lines = [];
    ...
    lines.push(`${BOLD}Queue${RESET}  pending ${YELLOW}${jobs.pending}${RESET}` +
        `  in-flight ${CYAN}${jobs.inFlight}${RESET}` +
        `  workers ${workers}`);
    lines.push(`${BOLD}Totals${RESET}  succeeded ${GREEN}${jobs.succeeded}${RESET}` +
