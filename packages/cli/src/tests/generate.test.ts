...
void describe('GenerateCommand', () => {
  ...
  void it('pluralizes name ending in x with -es for route path', async () => {
    await withTempDir(async (tmpDir) => {
      ...
      // toPlural('box') -> 'boxes'
      assert.ok(content.includes("@Controller('/api/boxes')"),
        `Expected route /api/boxes, got: ${content.slice(content.indexOf('@Controller'), content.indexOf('@Controller') + 40)}`);
    });
  });
  ...
  void it('pluralizes name ending in ch with -es for route path', async () => {
    await withTempDir(async (tmpDir) => {
      ...
      assert.ok(content.includes("@Controller('/api/benches')"),
        'Expected route /api/benches for name ending in ch');
    });
  });
  ...
  void it('pluralizes name ending in sh with -es for route path', async () => {
    await withTempDir(async (tmpDir) => {
      ...
      assert.ok(content.includes("@Controller('/api/flashes')"),
        'Expected route /api/flashes for name ending in sh');
    });
  });
  ...
  void it('pluralizes name ending in z with -es for route path', async () => {
    await withTempDir(async (tmpDir) => {
      ...
      assert.ok(content.includes("@Controller('/api/buzzes')"),
        'Expected route /api/buzzes for name ending in z');
    });
  });
  ...
  void it('pluralizes consonant+y name by replacing y with ies', async () => {
    await withTempDir(async (tmpDir) => {
      ...
      assert.ok(content.includes("@Controller('/api/categories')"),
        `Expected route /api/categories for consonant+y name, got content`);
    });
  });
  ...
  void it('pluralizes vowel+y name by adding s (not ies)', async () => {
    await withTempDir(async (tmpDir) => {
      ...
      assert.ok(content.includes("@Controller('/api/days')"),
        'Expected route /api/days for vowel+y name');
      // Should NOT use 'dies'
      assert.ok(!content.includes('dies'),
        'Should not use dies for vowel+y name');
    });
  });
  ...
  void it('converts camelCase name to kebab-case filename with PascalCase class', async () => {
    await withTempDir(async (tmpDir) => {
      ...
      assert.ok(content.includes('HelloworldController'),
        'Class name should be PascalCase: HelloworldController');
    });
  });
  ...
  void it('converts camelCase name to snake_case TABLE_NAME in repository', async () => {
    await withTempDir(async (tmpDir) => {
      ...
      assert.ok(content.includes("TABLE_NAME = 'blog_posts'"),
        `Expected TABLE_NAME = 'blog_posts' for camelCase repository`);
    });
  });
  ...
  void it('handles mixed underscore and digits in name', async () => {
    await withTempDir(async (tmpDir) => {
      ...
      assert.ok(content.includes('V2ApiController'),
        'Class name should be V2ApiController');
      // pluralName = toPlural('v2_api') -> 'v2_apis' (no kebab conversion on route)
      assert.ok(content.includes("@Controller('/api/v2_apis')"),
        'Expected route /api/v2_apis (pluralName preserves underscores)');
    });
  });
});
