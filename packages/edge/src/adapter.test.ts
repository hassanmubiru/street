...
describe('handleEdgeRequest', () => {
  it('routes a GET request through the middleware pipeline and returns JSON', async () => {
    ...
    app.use(async (ctx, next) => {
      if (ctx.path === '/ping') { ctx.json({ pong: true, method: ctx.method }); return; }
      await next();
    });

    const res = await handleEdgeRequest(new Request('https://edge.example/ping'), app);
    assert.equal(res.status, 200);
    const body = await res.json() as { pong: boolean; method: string };
  ...
  it('echoes a POST body', async () => {
    ...
    const res = await handleEdgeRequest(
      new Request('https://edge.example/echo', {
        method: 'POST',
    ...
    assert.equal(res.status, 200);
    const body = await res.json() as { received: { hello: string } };
