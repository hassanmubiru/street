...
const unownedNamespaceArb: fc.Arbitrary<string> = wordArb
  .map((w) => `not-${w}`)
  .filter((ns) => ns !== OWNED_NS);

/** A plugin name in a namespace the publisher OWNS (`acme/x` or `@acme/x`). */
const ownedNameArb: fc.Arbitrary<string> = fc
  .tuple(fc.boolean(), wordArb)
  .map(([scoped, w]) => (scoped ? `@${OWNED_NS}/${w}` : `${OWNED_NS}/${w}`));

/** A plugin name in a namespace the publisher does NOT own. */
const unownedNameArb: fc.Arbitrary<string> = fc
  .tuple(fc.boolean(), unownedNamespaceArb, wordArb)
  .map(([scoped, ns, w]) => (scoped ? `@${ns}/${w}` : `${ns}/${w}`));

/** Loose well-formed manifest fields (metadata-valid). */
interface ManifestSeed {
  name: string;
  version: string;
}

const ownedSeedArb: fc.Arbitrary<ManifestSeed> = fc.record({ name: ownedNameArb, version: versionArb });
const unownedSeedArb: fc.Arbitrary<ManifestSeed> = fc.record({ name: unownedNameArb, version: versionArb });
 ...
 */
const badCredentialArb: fc.Arbitrary<string | undefined> = fc.oneof(
  fc.constant<string | undefined>(undefined),
  fc.constantFrom('', '   ', 'Bearer', 'null'),
  fc.string({ maxLength: 40 }).filter((s) => s !== VALID_API_KEY),
);
...
function signedRequest(seed: ManifestSeed, privateKey: KeyObject, publicKeyPem: string) {
  const manifest = signManifest(
    { name: seed.name, version: seed.version, capabilities: [], dependencies: {}, permissions: [] } as PluginManifest,
    privateKey,
  );
...
function totalVersions(service: RegistryService): number {
...
describe('Property 13: publishing requires authentication and authorization', () => {
  it('rejects an unauthenticated publish (UNAUTHENTICATED) and leaves the store unchanged', () => {
    fc.assert(
      fc.property(ownedSeedArb, badCredentialArb, (seed, badKey) => {
        ...
        assert.equal((res as { code: string }).code, 'UNAUTHENTICATED');
  ...
  it('authentication is the first gate: a bad credential is rejected even for a malformed manifest', () => {
    fc.assert(
      fc.property(badCredentialArb, fc.constantFrom<unknown>(null, 42, 'str', {}, { name: '' }, undefined), (badKey, malformed) => {
        const { service, privateKey, publicKeyPem } = freshService();
        const before = totalVersions(service);
  ...
  it('rejects an authenticated-but-unauthorized publish (UNAUTHORIZED) and leaves the store unchanged', () => {
    fc.assert(
      fc.property(unownedSeedArb, (seed) => {
        const { service, privateKey, publicKeyPem } = freshService();
        const seeded = seedValidVersion(service, privateKey, publicKeyPem);
        const before = totalVersions(service);
        ...
        assert.equal((res as { code: string }).code, 'UNAUTHORIZED');
        assert.equal((res as { field?: string }).field, 'name');
