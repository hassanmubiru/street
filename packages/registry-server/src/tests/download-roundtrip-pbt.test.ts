...
import { signManifest, verifyManifest, manifestChecksum, type PluginManifest } from 'streetjs';
...
function signFor(base: WellFormed, privateKey: KeyObject): PluginManifest {
  return signManifest(
    {
      name: base.name,
...
describe('Property 9: download is a byte-faithful round trip', () => {
  it('download returns the published tarball bytes EXACTLY, with its recorded signature (Req 4.3)', () => {
    fc.assert(
      fc.property(wellFormedArb, tarballArb, (base, tarball) => {
        ...
        if (isRegistryError(download)) return; // narrow for TS
        ...
        assert.equal(
          ...
          download.tarballChecksum,
          'downloaded bytes must re-hash to the recorded checksum',
        );
      }),
      { numRuns: NUM_RUNS },
    );
  ...
  it('the downloaded package + recorded signature pass consumer-side integrity validation (Req 4.3)', () => {
    fc.assert(
      fc.property(wellFormedArb, tarballArb, (base, tarball) => {
        ...
        if (isRegistryError(download)) return;
    ...
    fc.assert(
      fc.property(
        ...
        (base, vA, vB, tarA, tarB) => {
          ...
          if (isRegistryError(dA) || isRegistryError(dB)) return;
