...
describe('JWT — fuzz & boundary testing', () => {
    ...
    it('rejects token with exp as string (type confusion)', () => {
        const malformedPayload = Buffer.from(JSON.stringify({ sub: 'user-1', exp: 'far-future' })).toString('base64url');
...
describe('Session Manager — fuzz & boundary testing', () => {
    ...
    it('handles large session data', () => {
        ...
        const blob = sm.encrypt(largeData);
        const decrypted = sm.decrypt(blob);
        assert.ok(decrypted !== null);
        assert.equal(decrypted['key_0'].length, 1000);
    });
...
describe('Vault — fuzz & boundary testing', () => {
    ...
    it('throws on corrupted ciphertext', () => {
        ...
        assert.throws(() => decryptSecret(buf.toString('base64'), kek), /decryption failed/);
    });
    it('throws on truncated ciphertext', () => {
        ...
        assert.throws(() => decryptSecret(truncated, kek), /too short/);
    });
    it('throws with wrong KEK', () => {
        const encrypted = encryptSecret('sensitive', kek);
        assert.throws(() => decryptSecret(encrypted, 'wrong-kek-1234567890!!!'), /decryption failed/);
    });
...
describe('Rate Limiter — boundary & saturation testing', () => {
    it('allows burst of requests then blocks', async () => {
        ...
        await assert.rejects(() => mw(makeCtx(), async () => { passed++; }), /Too Many Requests/);
        assert.equal(passed, 5);
        limiter.destroy();
    });
    ...
    it('handles concurrent rate limit checks', async () => {
        ...
        const ctx = makeCtx();
        ctx['headers'] = { 'x-forwarded-for': key };
        // Fire 10 concurrent requests at the same key
        const results = await Promise.allSettled(Array.from({ length: 10 }, () => mw(makeCtx(), async () => undefined)));
...
describe('Auth Middleware — security testing', () => {
    ...
    it('rejects missing Authorization header', async () => {
        ...
        await assert.rejects(() => mw(makeCtx(), async () => undefined), /Missing Bearer token/);
    });
    it('rejects non-Bearer authorization', async () => {
        const mw = authMiddleware(jwt);
        const ctx = makeCtx();
        ctx['headers'] = { authorization: 'Basic dGVzdDp0ZXN0' };
        await assert.rejects(() => mw(ctx, async () => undefined), /Missing Bearer token/);
    });
    it('rejects invalid token', async () => {
        const mw = authMiddleware(jwt);
        const ctx = makeCtx();
        ctx['headers'] = { authorization: 'Bearer invalid.token.here' };
        await assert.rejects(() => mw(ctx, async () => undefined), /Invalid or expired token/);
    });
    ...
    it('requireRoles rejects without auth', async () => {
        ...
        await assert.rejects(() => mw(makeCtx(), async () => undefined), /Unauthorized/);
    });
    it('requireRoles rejects insufficient permissions', async () => {
        const mw = requireRoles('admin');
        const ctx = makeCtx();
        ctx.user = { id: 'u1', email: 'u@b.com', roles: ['user'] };
        await assert.rejects(() => mw(ctx, async () => undefined), /Insufficient permissions/);
    });
