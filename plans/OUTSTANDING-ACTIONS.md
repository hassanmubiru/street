...
|---|---|---|---|---|
| 1 | ✅ **Done (operator-verified 2026-06-27)** — branch protection on `main`: 11 required checks, `require_code_owner_reviews: true`, `required_approving_review_count: 1`, `required_linear_history: true`, `allow_force_pushes: false`, `enforce_admins: true` (confirmed by the branch-protection API response). *(Live set = 11 verified-present checks; the security-workflow contexts in `repository-settings.json` to be added after they report on a PR.)* | [OPERATOR] | `security/BRANCH-PROTECTION-REVIEW.md`, `OPENSSF-REVIEW.md`, `SLSA-ASSESSMENT.md` | Settings applied + a blocked direct push confirms |
...
| 4 | ◑ **Repo-safe; physical relocation pending (operator)** — confirmed all key files are **gitignored + untracked** (cannot be re-committed; `secrets-guard` would block). 3 private keys still on disk in the working tree: `street-signing.key.pem` (rotated/dead), `keys/street-signing-2026.key.pem`, `packages/plugin-marzpay/signing-key.pkcs8.pem`. **Operator:** move live keys to a secrets manager + `shred -u` the local copies. | [OPERATOR] | `KEY-ROTATION-RUNBOOK.md` §8, `SECURITY-CLASSIFICATION.md` | No key files in the working tree |
