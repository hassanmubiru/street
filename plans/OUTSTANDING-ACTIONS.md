...
| 4 | ✅ **Done (operator-verified 2026-06-27)** — no private key material in the working tree: `find` for `*.key.pem`/`*.pkcs8.pem`/`*.key`/`*.p12`/`*.pfx` is empty; `street-signing.key.pem`, `keys/street-signing-2026.key.pem`, `packages/plugin-marzpay/signing-key.pkcs8.pem` all removed (only public `*.pub.pem` remain); none tracked. *Operator to confirm live keys persist as CI secrets / in secrets manager (`gh secret list`).* | [OPERATOR] | `KEY-ROTATION-RUNBOOK.md` §8, `SECURITY-CLASSIFICATION.md` | No key files in the working tree |
