...
function makeMockReq(body?: object): Readable {
...
function simulateParseBodyLifecycle(
  req: Readable,
  maxBytes = 1024 * 1024
): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    ...
    const onEnd = (): void => resolve(undefined);
    const onError = (err: Error): void => reject(err);
    const onAborted = (): void => reject(new Error('Request aborted'));
  ...
  it(`does not leak listeners after ${ITERATIONS} parseBody cycles`, async () => {
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      assert.equal(
        ...
        `data listener leak at iteration ${i}`
      );
      assert.equal(
        req.listenerCount('end'), 0,
        `end listener leak at iteration ${i}`
      );
      assert.equal(
        req.listenerCount('error'), 0,
        `error listener leak at iteration ${i}`
      );
      assert.equal(
        req.listenerCount('aborted'), 0,
        `aborted listener leak at iteration ${i}`
      );
    }
  });
...
describe('PgPool — acquire/release stress test', () => {
  ...
  it(`no connection leak after ${ITERATIONS} acquire/release cycles`, async () => {
    ...
    assert.equal((pool as any).waitQueue.length, 0, 'waitQueue not empty');
  ...
  it(`no listener accumulation on pool after ${ITERATIONS} query() calls`, async () => {
    ...
    assert.ok(
      leaked <= 2, // allow small noise from Node.js internals
