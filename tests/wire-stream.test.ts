...
function createMockSocket() {
  const socket = new EventEmitter() as EventEmitter & {
    ...
    resume: ReturnType<typeof mock.fn>;
  };
  socket.setKeepAlive = () => {};
  socket.setNoDelay = () => {};
  socket.destroy = () => {};
...
function singleColRowDesc(colName: string, typeOid = 25): Buffer {
...
function buildDataRow(cols: (string | null)[]): Buffer {
  const colBuffers: Buffer[] = [];
...
describe('StreetPostgresWireStream', () => {
  ...
  it('can push multiple rows and drain them via pipe', async () => {
    ...
    stream.on('data', (row: unknown) => collected.push(row));
  ...
  it('multiple rows with varied data types', async () => {
    ...
    assert.equal(collected.length, 3);
    assert.equal((collected[0] as Record<string, string | null>)['name'], 'Alice');
    assert.equal((collected[2] as Record<string, string | null>)['name'], null);
  });
});
...
describe('PgConnection.queryStream', () => {
  ...
  it('handles rows with null values', async () => {
    ...
    assert.equal(collected.length, 1);
    assert.equal((collected[0] as Record<string, string | null>)['n'], null);
  });
  ...
  it('handles rows with multiple columns', async () => {
    ...
    assert.equal(collected.length, 1);
    const row = collected[0] as Record<string, string | null>;
  ...
  it('emits error when server sends ErrorResponse', async () => {
    ...
    const errBody = Buffer.from(
      'SERROR\0Mrelation "nonexistent" does not exist\0\0',
      'utf8'
    );
  ...
  it('does not interfere with subsequent simple queries after stream', async () => {
    ...
    stream1.on('data', (row: unknown) => collected1.push(row));
    ...
    socket.emit('data', response);
    await new Promise<void>((resolve) => stream1.on('end', resolve));
