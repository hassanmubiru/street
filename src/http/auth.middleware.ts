...
export function authMiddleware(jwt: JwtService): MiddlewareFn {
...
export function requireRoles(...roles: string[]): MiddlewareFn {
  return async (ctx: StreetContext, next: () => Promise<void>): Promise<void> => {
    if (!ctx.user) {
...
/** CORS middleware */
export function corsMiddleware(origins: string[] = ['*']): MiddlewareFn {
  return async (ctx: StreetContext, next: () => Promise<void>): Promise<void> => {
