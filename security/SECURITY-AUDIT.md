...
| F-3 | High | Process gap: signing happens **outside the enforced CI gate**. A general CI workflow (`publish-plugins.yml`) signs **all 21 plugins** from `STREET_PLUGIN_SIGNING_KEY` and fails loud unless each manifest verifies against the official key (`sign-htmx.yml` is a separate one-shot for the last plugin that lacked a committed manifest). Yet `@streetjs/plugin-marzpay@1.1.0`'s committed manifest is signed with `7de6474b…` (the **on-disk** `street-signing.key.pem`), which does **not** match the official anchor and would **fail** `publish-plugins.yml`'s own verify-against-official step — so it was signed during a **manual local publish**, bypassing the CI gate and putting a long-lived key on a workstation. | Open | `publish-plugins.yml` matrix includes all 21 plugins + a "Verify packed manifest is officially signed" step; `packages/plugin-marzpay/manifest.pub` = `7de6474b…` ≠ official `df5e2726…`. |
